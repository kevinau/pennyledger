...
public class MboxIterator implements Iterable<CharBufferWrapper>, Closeable {
    ...
    private final FileChannel mboxChannel;
    ...
    private long eofOffset = 0;
    
    private MboxIterator(final Path mboxPath,
        ...
        this.theFile = new FileInputStream(mboxPath.toFile());
        this.mboxChannel = theFile.getChannel();
        this.byteBuffer = mboxChannel.map(FileChannel.MapMode.READ_ONLY, startOffset, mboxChannel.size() - startOffset);
        eofOffset = mboxChannel.size();
        initMboxIterator();
    }
    ...
    public static Builder fromFile(Path filePath, long startOffset) {
        return new Builder(filePath, startOffset);
    }
    public static Builder fromFile(String file, long startOffset) {
        return new Builder(file, startOffset);
    }
    public static class Builder {
        ...
        private String regexpPattern = FromLinePatterns.LENIENT;
        ...
        private Builder(String fileName, long startOffset) {
            this(Paths.get(fileName), startOffset);
        }
        private Builder(Path path, long startOffset) {
            this.path = path;
        ...
        public MboxIterator build() throws FileNotFoundException, IOException {
            return new MboxIterator(path, startOffset, charset, regexpPattern, flags, maxMessageSize);
        }
    }
    /**
