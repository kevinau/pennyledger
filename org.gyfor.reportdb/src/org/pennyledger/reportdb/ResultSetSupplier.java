...
import org.pennyledger.report.ReportDetail;
...
public class ResultSetSupplier<P> implements ResettableSupplier<P>, AutoCloseable {
  ...
  private void buildSelect(Class<?> klass, IEntityPlan<?> template) {
    ...
    for (Field field : fields) {
      ...
      IItemPlan<? super Object> fieldPlan = template.getFieldPlan(fieldName);
      if (fieldPlan == null) {
  ...
  public ReportDetail simpleDetail(String... names) {
    ...
    for (String name : names) {
      IItemPlan<?> fieldPlan = entityPlan.getFieldPlan(name);
      if (fieldPlan == null) {
